...
def check_fibonacci(data: Sequence[int]) -> bool:
    ...
    while new_el < last_data_el: #the loop will do the work until the new element is smaller than the last element
        new_el = true_fib_range[i - 1] + true_fib_range[i - 2] #since in the Fibonacci sequence, the number coming after 0 and 1 is the sum of the previous two, so here we find a new element through the indexes
